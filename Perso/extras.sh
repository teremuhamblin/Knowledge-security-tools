#!/usr/bin/env bash
# ===================================================================
# 🔧 Knowledge Security Tools – Misc / Catch-All Installer v3.5 (TUI Edition)
# Author: The MadDoG.tmdg
# Optimized with fzf Interactive Edition
# ===================================================================
set -euo pipefail
...
# 📁 Logs
LOGFILE="${LOGFILE:-$HOME/kst-misc-install.log}"
...
install_git() {
  local repo="$1" dst="$2"
  mkdir -p /opt/kst-misc
  [[ -d "/opt/kst-misc/$dst" ]] && skip "$dst" && return
  git clone --depth=1 "$repo" "/opt/kst-misc/$dst" &>/dev/null && ok "$dst (git)" || fail "$dst (git)"
}

bootstrap_basics() {
  say "Installing base dependencies..."
  case "$PKG" in
    apt) apt update -y &>/dev/null; apt install -y python3 python3-pip git curl wget golang-go fzf &>/dev/null ;;
    pacman) pacman -Sy --noconfirm &>/dev/null; pacman -S --noconfirm --needed python python-pip git curl wget go fzf &>/dev/null ;;
    dnf) dnf install -y python3 python3-pip git curl wget golang fzf &>/dev/null ;;
  esac
...
# ===================================================================
select_tools() {
  say "Select Misc / Catch-All tools to install:"
  local choices tools
...
EOF
  ...
  tools=$(echo "$choices" | fzf --multi --border --prompt="Misc > " --header="Select tools to install" --height=15 --reverse)
...
for tool in $selected; do
  case "$tool" in
    ...
    "EyeWitness (Git + internal setup)")
      ...
      bash /opt/kst-misc/EyeWitness/Python/setup/setup.sh -y &>/dev/null && ok "EyeWitness (compiled)" || fail "EyeWitness setup"
      ;;
